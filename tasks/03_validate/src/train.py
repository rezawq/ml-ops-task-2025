...
def read_parquet_and_split(spark, parquet_path: str, train_ratio: float = 0.8):
    ...
    try:
        ...
        train_df, test_df = first_1000_df.randomSplit([train_ratio, 1 - train_ratio], seed=42)
...
def train_model(train_df, test_df, feature_cols, model_type="rf", run_name="fraud_detection_model"):
    ...
    try:
        ...
        with mlflow.start_run(run_name=run_name) as run:
            ...
            # Оцениваем нижнюю и верхнюю границу параметра f1 с помощью bootstrap
            bootstrap_iterations = 100
            f1_scores = []

            print("DEBUG: Начинаем bootstrap для оценки доверительных интервалов F1")
            for i in range(bootstrap_iterations):
                # Создаем бутстреп-выборку с повторением
                print(f"DEBUG: Итерация {i}. Создаем бутстреп-выборку с повторением ")
                bootstrap_sample = test_df.sample(fraction=1.0, withReplacement=True, seed=42 + i)

                # Делаем предсказания на бутстреп-выборке
                print(f"DEBUG: Итерация {i}. Делаем предсказания на бутстреп-выборке ")
                bootstrap_predictions = best_model.transform(bootstrap_sample)

                # Вычисляем F1-score для текущей выборки
                print(f"DEBUG: Итерация {i}. Вычисляем F1-score для текущей выборки")
                current_f1 = evaluator_f1.evaluate(bootstrap_predictions)
            ...
            # Рассчитываем статистики без numpy
            if len(f1_scores) == 0:
                f1_mean = 0
                f1_variance = 0
                f1_std = 0
                ...
                f1_variance = sum((x - f1_mean) ** 2 for x in f1_scores) / len(f1_scores) if len(f1_scores) > 0 else 0
                f1_std = f1_variance ** 0.5 if f1_variance > 0 else 0

            confidence = 0.95

            # Вычисляем доверительный интервал (процентильный метод)
            f1_scores_sorted = sorted(f1_scores)
            lower_idx = int((1 - confidence) / 2 * bootstrap_iterations)
            upper_idx = int((1 + confidence) / 2 * bootstrap_iterations)
            f1_lower = f1_scores_sorted[lower_idx]
            f1_upper = f1_scores_sorted[upper_idx]

            # Логируем результаты в MLflow
            print(f"DEBUG: F1 доверительный интервал ({confidence * 100}%): [{f1_lower:.4f}, {f1_upper:.4f}]")
            mlflow.log_metric("f1_lower", f1_lower)
            mlflow.log_metric("f1_upper", f1_upper)
...
def compare_and_register_model(new_metrics, experiment_name):
    ...
    if not best_metrics:
        ...
        # Сравниваем на основе f1
        print(f"DEBUG: Сравниваем метрики - текущий f1: {best_metrics['f1']}, новый f1: {new_metrics['f1']}")
